    ...
    (function(){
		...
		kvm.stackViewRefresh = function() {
			...
			var basewas=kvm.forth.base;
			if(count>0) for(var i=0;i<count;i++){
				if (typeof(kvm.stack()[i])=="number") {
					stackView += ':'+i+'\t'+kvm.stack()[i]+'\t'+kvm.stack()[i].toString(16)+'h';
				} else {
					stackView += ':'+i+'\t'+kvm.stack()[i]+'\t';
				}
				kvm.stack().push(kvm.stack()[i]);
				kvm.dictate("js> mytypeof(pop())");
